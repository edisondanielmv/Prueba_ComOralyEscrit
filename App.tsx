    ...
    try {
        if (question.type === QuestionType.MULTIPLE_CHOICE) {
            if (question.options && typeof question.correctOptionIndex === 'number') {
                ...
                // Si answerValue está vacío (por tiempo agotado), falla naturalmente y da 0 puntos.
            ...
            // LÓGICA DE TEXTO ABIERTO
            
            // Si la respuesta está vacía (Tiempo agotado o dejada en blanco explícitamente)
            if (!answerValue || answerValue.trim().length === 0) {
                 safeResponse.pointsEarned = 0;
                 safeResponse.aiAnalysis = {
                    questionId: question.id,
                    score: 0,
                    feedback: "Pregunta sin responder (0 puntos asignados automáticamente)."
                 };
                 return safeResponse;
            }
            ...
            const timeoutPromise = new Promise<any>((resolve) => {
                setTimeout(() => resolve({ score: 0, feedback: "Tiempo de espera IA agotado. Revise su conexión." }), 45000);
            });
    ...
    try {
      ...
      for (let i = 0; i < examQuestions.length; i += BATCH_SIZE) {
         ...
         const batchResults = await Promise.all(batch.map(q => {
             // Si no hay respuesta en el array (porque no se marcó nada), pasamos cadena vacía.
      ...
      const fallbackDetails = examQuestions.map(q => ({
          ...
          pointsEarned: 0,
          maxPoints: q.points,
          aiAnalysis: { score: 0, feedback: "Error crítico durante la evaluación." }
      }));
